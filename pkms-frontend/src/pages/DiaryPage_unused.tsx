...
import { AudioRecorder } from '../components/file/AudioRecorder';
...
import { ComprehensiveHabitTracker } from '../components/diary/ComprehensiveHabitTracker';
...
export function DiaryPage() {
  ...
  const handleAudioRecordingComplete = async (audioBlob: Blob, duration: number) => {
    try {
      ...
      // Add the audio file to the diary entry form
      // You can store it in form state or upload it immediately
      // For now, let's add it to the form as a note
      const currentContent = form.values.content || '';
      const audioNote = `\n\n🎤 Voice Note (${Math.round(duration)}s): ${filename}\n`;
      form.setFieldValue('content', currentContent + audioNote);
      
      setShowAudioRecorder(false);
      notifications.show({
        title: 'Audio Recorded',
        message: `Voice note recorded successfully (${Math.round(duration)}s)`,
        color: 'green'
      });
    } catch (error) {
                  ...
                  <Tabs value={searchMode} onChange={(value) => setSearchMode(value as 'filter' | 'search')}>
                    ...
                    <Tabs.Panel value="search" pt="sm">
                      <UnifiedSearchEmbedded
                        defaultModules={['notes', 'documents', 'todos', 'archive', 'folders', 'projects']}
                        includeDiary={false}
                        showModuleSelector={true}
                        showSearchTypeToggle={true}
                        onResultClick={(result) => {
                          // Navigate to the found item
                          notifications.show({
                            title: 'Item Found',
                            message: `Found ${result.module}: ${result.title}`,
                            color: 'blue'
                          });
                        }}
                        emptyMessage="No items found. Try different search terms or adjust your filters."
                        resultsPerPage={15}
                        showPagination={true}
                      />
                    </Tabs.Panel>

...
async def get_diary_entry_by_id(
    ...
    try:
        ...
        response = DiaryEntryResponse(
            ...
            created_at=entry.created_at,
            updated_at=entry.updated_at,
            file_count=len(entry.media) if hasattr(entry, 'media') else entry.file_count,
            tags=tags,
            content_length=entry.content_length,
        )
        return response
...
async def update_diary_entry(
    ...
    return DiaryEntryResponse(
        ...
        created_at=entry.created_at,
        updated_at=entry.updated_at,
        file_count=len(entry.media) if hasattr(entry, 'media') else 0,
        tags=await _get_entry_tags(db, entry.uuid),
        content_length=entry.content_length
    )
...
async def get_entry_files(
    ...
    # Serialize ORM objects to DiaryFileResponse
    return [
        DiaryFileResponse(
            uuid=media.uuid,
...
async def commit_diary_file_upload(
    ...
    try:
        ...
        logger.info(f"Diary media committed successfully: {diary_file.filename}")
        
        return DiaryFileResponse(
            uuid=diary_file.uuid,
            entry_id=diary_file.diary_entry_uuid,
            filename_encrypted=diary_file.filename,
            mime_type=diary_file.mime_type,
            size_bytes=diary_file.file_size,
...
async def download_diary_file(
    ...
    try:
        ...
        logger.exception(f"Error downloading diary media {file_uuid}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_calendar_data(
    ...
    file_count_subquery = (
        ...
        .where(
            and_(
                DiaryFile.created_by == current_user.uuid,

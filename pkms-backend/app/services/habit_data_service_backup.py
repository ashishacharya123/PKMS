...
class HabitDataService:
    ...
    # This service should only handle CRUD operations, not analytics
        ...
        It's designed to give users actionable insights into their personal development journey.
        
        ANALYTICS PROVIDED:
        - Mood analysis with distribution patterns and trend tracking
        - Habit correlation analysis (e.g., sleep quality vs mood, exercise vs stress)
        - Financial wellness tracking (income, expenses, savings trends)
        - Multi-dimensional wellness scoring system
        - Personalized insights and recommendations
        - Long-term trend analysis (3-month, 6-month, yearly comparisons)
        
        DATA SOURCES:
        - Diary entries: mood, content analysis, entry patterns
        - Daily metadata: habit tracking, financial data, wellness metrics
        - Historical trends: long-term pattern analysis
        - Correlation analysis: cross-metric relationships
        
        WELLNESS DIMENSIONS ANALYZED:
        1. Emotional Wellness: mood patterns, emotional stability, stress levels
        2. Physical Wellness: exercise habits, sleep quality, stress management
        3. Mental Wellness: meditation practice, screen time, focus metrics
        4. Financial Wellness: income stability, expense patterns, savings rate
        5. Social Wellness: social activity patterns, relationship insights
        6. Overall Wellness Score: composite score across all dimensions
        
        Args:
            db: Database session for data retrieval
            user_uuid: Unique identifier for the user
            days: Analysis period in days (default: 30, max recommended: 365)
            
        Returns:
            WellnessStats: Comprehensive analytics object containing:
                - Period metadata (start/end dates, total days, data coverage)
                - Mood analytics (average, trends, distribution)
                - Habit analytics (sleep, stress, exercise, meditation, screen time)
                - Financial analytics (income, expenses, savings, trends)
                - Wellness scoring (overall score, component breakdowns)
                - Trend analysis (short-term and long-term patterns)
                - Correlation insights (habit relationships and impacts)
                - Personalized recommendations based on data patterns
                
        Raises:
            HTTPException: If user not found or database errors occur
            ValueError: If days parameter is invalid (< 1 or > 365)
            
        Example:
            ```python
            # Get 30-day wellness analysis
            stats = await DiaryMetadataService.get_wellness_stats(db, user_uuid, 30)
            ...
            total_savings = stats.net_savings
            daily_income_avg = stats.average_daily_income
            
            # Access wellness scoring
            overall_score = stats.overall_wellness_score
            score_breakdown = stats.score_components
            ```
            
        Note:
            This function performs intensive analytics calculations and may take
            2-5 seconds for large datasets. Consider caching results for
        ...
        # ========================================================================
        # DAILY METADATA PROCESSING FOR COMPREHENSIVE WELLNESS ANALYTICS
        # ========================================================================
        # This section processes daily metadata records to build a comprehensive
        # dataset for multi-dimensional wellness analysis. We extract and structure
        # data from multiple sources to enable correlation analysis and trend tracking.
        
        # Parse metrics from JSON and build comprehensive daily dataset
        # Structure: {date_str: {mood, metrics_dict, income, expense}}
        # This structure enables:
        # 1. Habit correlation analysis (sleep vs mood, exercise vs stress)
        # 2. Financial wellness tracking (income/expense trends, savings rate)
        # 3. Multi-dimensional wellness scoring across all life areas
        # 4. Personalized insights based on cross-metric relationships
        daily_data = {}
        
        for record in metadata_records:
            date_str = record.date.strftime("%Y-%m-%d")
            
            # Extract habit tracking data from JSON
            # default_habits_json contains: sleep, stress, exercise, meditation, screen_time
            # These metrics are used for habit correlation analysis and wellness scoring
            try:
                metrics = json.loads(record.default_habits_json) if record.default_habits_json else {}
            except json.JSONDecodeError:
                metrics = {}
            
            # Build comprehensive daily data structure for analytics
            daily_data[date_str] = {
                "metrics": metrics,  # Habit tracking data for correlation analysis
                "mood": None,        # Will be filled from diary entries in next loop
                "income": float(record.daily_income or 0),   # Financial wellness tracking
                "expense": float(record.daily_expense or 0), # Financial wellness tracking
            }
        ...
        # ========================================================================
        # Process financial data using model fields (daily_income, daily_expense)
        # instead of metrics dict to ensure accurate financial trend analysis
        financial_trend = []
        total_income = 0.0
        total_expense = 0.0
        ...
        for date_str, data in sorted(daily_data.items()):
            # Use direct model fields for accurate financial tracking
            daily_income = data.get("income", 0.0)
            daily_expense = data.get("expense", 0.0)
    ...
    async def update_daily_tracking(
        ...
        """
        Update daily habit tracking data with proper schema preservation.
        
        Handles both default habits (simple dict) and defined habits (complex schema with streaks).
        Preserves existing data structure to avoid breaking analytics and wellness computations.
        """
        from app.services.habit_config_service import habit_config_service
        ...
        if config_type == "default_habits":
            # Preserve existing dict schema
            existing = {}
            try:
                existing = json.loads(getattr(snapshot, "default_habits_json") or "{}")
            except Exception:
                existing = {}
            merged = {**existing, **{k: v for k, v in tracking_data.items() if k in valid_items}}
            snapshot.default_habits_json = json.dumps(merged)
        else:
            # Preserve defined_habits schema: {habits, streaks, units, first_tracked, last_updated}
            current = {}
            try:
                current = json.loads(getattr(snapshot, "defined_habits_json") or "{}")
            except Exception:
                current = {}
            habits = current.get("habits", {})
            habits.update({k: v for k, v in tracking_data.items() if k in valid_items})
            current["habits"] = habits
            current["units"] = units or current.get("units", {})
            current["first_tracked"] = current.get("first_tracked", {})
            current["last_updated"] = datetime.now(NEPAL_TZ).isoformat()
            snapshot.defined_habits_json = json.dumps(current)
        await db.commit()
        await db.refresh(snapshot)
